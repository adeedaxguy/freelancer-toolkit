...
export default function ToolPageShell({ tool, children, variantLabel }: ToolPageShellProps) {
  ...
  const answerBullets = tool.answerBox?.bullets ?? []
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
      ...
      <div className="mb-7">
        ...
        <div className="mt-4 rounded-xl border border-brand-100 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wider text-brand-700">Short answer</p>
          <p className="mt-2 text-sm leading-6 text-gray-600">
            {tool.answerBox?.short ?? (
              <>
          ...
          {answerBullets.length > 0 && (
            <ul className="mt-3 grid gap-2 text-xs leading-5 text-gray-600 sm:grid-cols-2">
              {answerBullets.map((bullet) => (
                <li key={bullet} className="rounded-lg bg-brand-50 px-3 py-2">
                  {bullet}
                </li>
              ))}
            </ul>
