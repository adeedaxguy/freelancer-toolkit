...
export async function generateMetadata({ params }: { params: { variant: string } }): Promise<Metadata> {
  ...
  if (!variant) return {}
  return {
    title: { absolute: compactSeoTitle(`Image Compressor — ${variant.label}`) },
    description: compactSeoDescription(`Free online image compressor ${variant.label.toLowerCase()}. Reduce file size without losing quality. Runs in your browser — no upload needed.`),
