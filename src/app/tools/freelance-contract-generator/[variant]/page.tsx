...
export async function generateMetadata({ params }: { params: { variant: string } }): Promise<Metadata> {
  ...
  if (!variant) return {}
  return {
    title: { absolute: compactSeoTitle(`Freelance Contract Generator ${variant.label}`) },
    description: compactSeoDescription(`Generate a free, professional freelance contract ${variant.label.toLowerCase()}. Customize scope, payment terms, revision policy, and IP ownership — download-ready in minutes.`),
