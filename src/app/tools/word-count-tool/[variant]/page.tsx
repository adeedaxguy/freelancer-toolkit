...
export async function generateMetadata({ params }: { params: { variant: string } }): Promise<Metadata> {
  ...
  if (!variant) return {}
  return {
    title: { absolute: compactSeoTitle(`Word Count Tool — ${variant.label}`) },
    description: compactSeoDescription(`Free word counter and readability checker ${variant.label.toLowerCase()}. Instant word count, character count, reading time, and Flesch readability score.`),
