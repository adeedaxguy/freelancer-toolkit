...
2. Check `seo/opportunities.md` first. If Ahrefs/GSC exports are available, run `npm run seo:opportunities` before choosing the keyword.
3. Check Google Search Console context if supplied, and infer opportunity from impressions, CTR, and position.
4. Research live SERP intent and competitors. For volatile facts like marketplace fees, taxes, visa/photo rules, and platform policies, verify with current official sources.
5. Choose the content role: pillar page, cluster article, comparison page, how-to guide, tool guide, or location-specific page.
6. Build the funnel path: search intent -> answer -> proof/examples -> tool CTA -> related tools/posts.
7. Write metadata first: `title`, optional `seoTitle`, `description`, optional `seoDescription`, `date`, `publishDate`, `status: published`, `tags`, and `author`.
8. Write the article in MDX with a clear H1, useful intro, scannable H2/H3s, tables/checklists where helpful, and a FAQ section when the topic has question intent.
9. Add internal links to relevant tool pages, category hubs, and supporting blog posts. Link to the primary tool early and again near the CTA.
10. Add external authority links only when they improve trust or source volatile claims.
11. Run `npm run seo:qa`, fix any errors, then run `npm run build`.
12. Commit only the intended files, push `origin main`, wait for Vercel, verify the live URL returns 200, and confirm it appears in `https://freeltools.com/sitemap.xml`.
...
- Ahrefs data should be used when available for keyword difficulty, competing pages, content gaps, top pages, and backlink opportunities. Save exports in `seo/exports/` and generate the report before writing.
